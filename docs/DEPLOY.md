...
Скрипт сохраняет `data/*.json`, делает `git fetch` + `git reset --hard
origin/main`, возвращает данные на место, ставит production-зависимости,
перезапускает сервис и проверяет, что сайт отвечает 200. При неудаче печатает
последние 30 строк журнала и возвращает ненулевой код.

⚠️ `git reset --hard` затирает локальные правки на сервере. Правьте код только
через репозиторий. Файл `.env` не отслеживается git и не трогается.

**Прайс и каталоги живут на сервере.** `data/prices.json`, `data/radio.json`
и `data/dragmet.json` заказчик правит через админку, поэтому серверная версия
считается главной: скрипт сохраняет эти файлы до сброса и кладёт обратно
после. Если в репозитории они изменились, в выводе появится строка вида

```
    prices.json: в репозитории другая версия, оставляю серверную
```

Это не ошибка, а напоминание: правки из репозитория не накатились. Чтобы
взять версию из git и затереть серверную, нужен явный ключ:

```bash
ssh mbaza55 mbaza55-deploy --with-data
```

Предыдущая версия скрипта лежит рядом: `/usr/local/bin/mbaza55-deploy.bak-2026-09-05`.
