...
Сертификат ещё **не выпущен**: на момент развёртывания домен `mbaza55.ru` не
резолвился. Сейчас сайт работает по HTTP и доступен по IP.

Порядок действий:

1. В DNS-зоне домена создать A-записи:
...
2. Дождаться распространения (обычно 15 минут – 2 часа), проверить:

   ```bash
   ssh mbaza55 "getent hosts mbaza55.ru www.mbaza55.ru"
   ```

3. Выпустить сертификат (укажите реальный e-mail для уведомлений об истечении):

   ```bash
   ssh mbaza55 "CERT_EMAIL=you@example.com mbaza55-tls"
   ```

Скрипт сам проверит, что домен указывает на этот сервер, выпустит сертификат на
оба имени, включит редирект с HTTP на HTTPS и перезагрузит nginx. Автопродление
уже работает через `certbot.timer`.
...
| Задача | Статус |
|--------|--------|
| A-записи `mbaza55.ru` и `www` на `77.91.112.185` | ждёт вас |
| Выпуск TLS-сертификата (`mbaza55-tls`) | после DNS |
| Реальный e-mail для Let's Encrypt | ждёт вас |
